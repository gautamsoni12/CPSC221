...
// END PA 4 FUNCTIONS
  ...
  private:
    ...
    void buildHelp(PNG const & source, int resolution, QuadtreeNode* subtree);

    QuadtreeNode* copy(QuadtreeNode* someroot);

    void clearTree(QuadtreeNode*& node);

    RGBAPixel getPixHelp(int x, int y, QuadtreeNode* subtree) const;

    bool inRange (int x, int y, QuadtreeNode* subtree) const;

    void rotateClockWise (QuadtreeNode* subtree);

    void prune (int tolerance, QuadtreeNode * subtree);

    int difference ( QuadtreeNode * a, QuadtreeNode *b ) const;

    int pruneSize ( int tolerance, QuadtreeNode* subtree) const;

    int idealPrune(int numLeaves, int n, int interval) const;

    bool checkT( QuadtreeNode* subtree, QuadtreeNode* avgRoot, int avg) const;
